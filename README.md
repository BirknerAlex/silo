...
(`pg_advisory_xact_lock`) keyed on the index group. That closes the race
the MVP documented, where two concurrent publishes each regenerated the
index from their own view of the bucket and the loser's package silently
vanished from the index that won.
...
A publish used to LIST the bucket and GET every package just to find out
what was already there. The `packages` table replaces that: index
regeneration reads rows. For apk and npm the publish path touches object
storage exactly twice (write the package, write the index), and
`silo list` never touches it at all.
