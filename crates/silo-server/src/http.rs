...
async fn serve_package(
    ...
    // Without a configured upstream, there's nothing a storage miss could
    // ever pull through, so this repo/channel/format gets the old,
    // pre-pull-through fast path straight back: no `head` round trip up
    // front, since a presigned URL is generated whether or not the
    // object actually exists anyway — a miss just 404s downstream of the
    // redirect, exactly as it always did. This DB lookup runs in its
    // place on every download regardless, but it's a pooled query
    // against silo's own database rather than a call out to the object
    // store, which is the round trip actually worth avoiding on the
    // common path — most repos never configure an upstream at all.
    let has_upstream = match silo_core::pull_through::select_upstream(
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            let result = (StatusCode::INTERNAL_SERVER_ERROR, "storage error").into_response();
            state
                .metrics
                .http_requests
                .with_label_values(&[&surface, result.status().as_str()])
    ...
    let result = if !has_upstream {
        serve_existing_package(state, key, format, auth, repo, channel).await
    } else {
        // An upstream applies, so a storage miss might still be
        // servable via pull-through — worth the extra `head` round trip
        // to find out before committing to a redirect that would
        // otherwise just 404.
...
///
/// Real npm clients essentially never fetch this directly; `add-upstream`'s
/// reachability probe hits `-/ping` (see [`NpmRequest::Ping`]) rather than
/// the bare root, precisely because the root isn't reliably JSON across
/// real registries. This route exists for whatever *does* still fetch the
/// root — `npm ping` predates `-/ping` on some clients, and it's a
/// reasonable registry root to return regardless.
...
/// Splits an npm path into a packument, tarball, or health-check request.
///
...
/// point even for scoped names. `-/ping` is npm's standard health-check
/// endpoint — the one `add-upstream`'s reachability probe hits, since the
/// registry root itself isn't reliably JSON (Verdaccio, for one, serves
/// its browsable web UI as HTML there).
