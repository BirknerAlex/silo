//! First-run bootstrap.
//!
//! A registry with no credentials is unusable, and a registry with a
//! credential baked into the config file (the old `auth.publish_token`) is
//! one that everybody ships to production unchanged. So the first startup
//! against an empty database mints an admin token and an admin user,
//! prints them to stdout exactly once, and never does so again — the
//! secrets are only ever stored hashed.
//!
