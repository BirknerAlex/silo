...
use silo_db::audit::{self, Actor, ActorKind, AuditEntry, AuditQuery};
...
impl AdminServiceImpl {
    ...
    async fn add_upstream_inner(
        ...
        // From here on, a failure means the first sync didn't take —
        // which the proto contract for `add-upstream` says leaves
        // nothing behind, not a half-created upstream a client would
        // then have to notice and clean up itself via `remove-upstream`.
        if let Err(e) = self
            .first_sync_after_create(&req, format, row.id, fetched, &caller.actor)
            .await
        {
            if let Err(cleanup_err) = self.state.db.delete_upstream(row.id).await {
                tracing::error!(
                    error = %cleanup_err,
                    upstream_id = %row.id,
                    "failed to roll back upstream row after a failed first sync"
                );
            }
            return Err(e);
        }
    ...
    /// The persisting half of `add_upstream_inner`, after the row itself
    /// is inserted: stores what the reachability check already fetched,
    /// marks the sync successful, and rebuilds the index so the new
    /// upstream's packages show up immediately. Split out so the caller
    /// can roll the row back as a unit on any failure here.
    async fn first_sync_after_create(
        &self,
        req: &AddUpstreamRequest,
        format: PackageFormat,
        upstream_id: uuid::Uuid,
        fetched: Vec<silo_pkg::UpstreamPackage>,
        actor: &Actor,
    ) -> Result<(), Status> {
        if format != PackageFormat::Npm {
            ...
            self.state
                .db
                .replace_upstream_packages(upstream_id, &synced)
                .await
                .map_err(|e| Status::internal(e.to_string()))?;
        }
        self.state
            .db
            .record_upstream_sync_success(upstream_id)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
