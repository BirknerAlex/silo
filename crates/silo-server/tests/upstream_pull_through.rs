...
async fn npm_packument_miss_lazily_syncs_and_renders_from_the_upstream() {
    ...
    let mock = MockServer::start().await;
    // The reachability probe `add-upstream` does for npm.
    Mock::given(method("GET"))
        .and(path("/-/ping"))
...
/// twin need their own routes (see `get_npm_root`) or a bare-root request
/// (`npm ping` on some clients, or a health check) 404s unconditionally.
#[tokio::test]
...
/// `-/ping` is what `add-upstream`'s npm reachability probe actually hits
/// — including when the upstream is another silo — so silo has to answer
/// it itself, or npm pull-through can never work silo-to-silo.
#[tokio::test]
async fn the_npm_ping_endpoint_answers_ok() {
    let url = require_db!();
    let harness = Harness::new(&url).await;
    let repo = unique_repo("npmping");
    harness.db.set_repo_public(&repo, true).await.unwrap();

    let resp = get(&harness.state, &format!("/{repo}/stable/npm/-/ping")).await;
    assert_eq!(resp.status(), axum::http::StatusCode::OK);
    let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
        .await
        .unwrap();
    let doc: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    assert!(doc.is_object(), "GET -/ping -> {doc}");
}
