...
/// Regression test: index pruning used to delete the package files.
///
...
/// directory as its packument — so a sweep that only spared the index
/// objects it had just written removed the packages along with the stale
/// metadata. Nothing failed loudly: the index still listed packages whose
