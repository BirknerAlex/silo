...
pub struct Config {
    ...
    /// Superseded by `signing.gpg`. Kept only so `validate` can reject it
    /// by name instead of silently ignoring a configured signing key.
...
pub struct AuthConfig {
    ...
    /// Rejected at load time with a pointer at what replaced them.
    /// Silently ignoring a stale `publish_token` would leave an operator
    /// believing the server is locked down by a token it isn't reading.
    #[serde(default, skip_serializing)]
...
impl Config {
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        if self.auth.publish_token.is_some() || self.auth.read_token.is_some() {
            anyhow::bail!(
                "`auth.publish_token`/`auth.read_token` are no longer supported — tokens now live \
                 in the database. Remove them and use the token printed on first startup, or \
                 `silo token create`."
            );
        }
        if self.gpg.is_some() {
            anyhow::bail!("top-level `gpg:` moved under `signing:` — use `signing.gpg` instead");
        }
...
"#;
    ...
    #[test]
    fn rejects_the_old_hardcoded_token_config() {
    ...
    #[test]
    fn rejects_the_old_top_level_gpg_block() {
        let yaml = format!("{MINIMAL}\ngpg:\n  key_path: /etc/silo/key.asc\n");
        let cfg: Config = serde_yaml::from_str(&yaml).unwrap();
        let err = cfg.validate().unwrap_err().to_string();
        assert!(err.contains("signing.gpg"), "got: {err}");
    }
