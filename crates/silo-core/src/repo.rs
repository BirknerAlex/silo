...
//!
//! The MVP had a documented race: two concurrent publishes to one
//! repo/channel each regenerated the index from their own view of object
//! storage, and the loser's package silently vanished from the index that
//! won. The fix has two halves, and both live in this module:
//!
...
/// Legacy layout helpers, kept as the single place the RPM paths are
/// spelled out for callers that don't have a `ParsedPackage` in hand
/// (the HTTP surface builds keys from URL segments).
