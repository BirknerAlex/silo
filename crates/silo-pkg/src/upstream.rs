...
mod tests {
    ...
    #[test]
    fn only_the_configured_origin_is_same_origin() {
        let http = UpstreamHttp::new(reqwest::Client::new(), "https://example.com/repo");
        assert!(http.is_same_origin("https://example.com/repo/repodata/repomd.xml"));
        assert!(http.is_same_origin("https://example.com:443/other"));
        // Different host, scheme, or port must not receive the credential.
        assert!(!http.is_same_origin("https://cdn.example.com/pkg.rpm"));
        assert!(!http.is_same_origin("http://example.com/pkg.rpm"));
        assert!(!http.is_same_origin("https://example.com:8443/pkg.rpm"));
        // An unparseable target is treated as foreign.
        assert!(!http.is_same_origin("not a url"));
    }
