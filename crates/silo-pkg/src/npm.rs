...
/// Fetches the registry's `/-/ping` health endpoint and confirms it looks
/// like an npm registry — a JSON object response. Every npm-compatible
/// registry implements `-/ping`; the plain root URL doesn't work for
/// this the same way, since Verdaccio serves its browsable web UI as
/// HTML there by default. A 404 or an HTML error page fails this, which
/// is the point: it catches "this URL isn't an npm registry at all" at
/// `add-upstream` time rather than on the first install.
async fn probe_registry_root(http: &UpstreamHttp) -> Result<(), UpstreamError> {
    let bytes = http.get("-/ping").await?;
...
mod tests {
    ...
    #[test]
    fn falls_back_to_the_highest_stable_version_without_a_dist_tag() {
        let bytes = serde_json::to_vec(&json!({
            "versions": { "1.0.0": {}, "2.0.0": {}, "3.0.0-rc.1": {} }
        }))
        .unwrap();
        assert_eq!(latest_upstream_version(&bytes), Some("2.0.0".to_string()));

        let only_pre = serde_json::to_vec(&json!({ "versions": { "1.0.0-rc.1": {} } })).unwrap();
        assert_eq!(
            latest_upstream_version(&only_pre),
