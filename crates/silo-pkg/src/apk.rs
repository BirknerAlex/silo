...
pub fn parse_apkindex_tar_gz(
    ...
    for (start, end) in &members {
        ...
        for entry in entries {
            ...
            if path != "APKINDEX" && path != "./APKINDEX" {
                continue;
            }
...
fn tokenize_version(v: &str) -> Vec<Token> {
    ...
    while let Some(&c) = chars.peek() {
        if c.is_ascii_digit() {
            ...
            // Neither digit, separator, nor letter (e.g. `+`, `~`) — drop
            // it but still advance, or a character apk itself doesn't
            // treat as a token boundary would loop here forever.
            chars.next();
        }
    }
    tokens
