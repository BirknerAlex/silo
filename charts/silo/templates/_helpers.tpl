...
Neither is required when the config comes from `configOverride` or
`existingConfigSecret`: both carry their own `database.url`, and demanding
a second source for it would make a hand-written config impossible to
deploy.
*/}}
...
{{- else if not (or .Values.existingConfigSecret .Values.configOverride) -}}
